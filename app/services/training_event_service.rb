class TrainingEventService
  ...
  def create_training_event(current_user, employee_ids, certification_type_ids, certification_date, trainer)
    ...
    employee_ids.each do |employee_id|
      certification_type_ids.each do |certification_type_id|
        ...
        if certification.present?
          success = _recertify(certification, certification_date, trainer)
        else
          certification = _certify(certification_date, certification_type_id, current_user, employee_id, trainer)
  ...
  def _certify(certification_date, certification_type_id, current_user, employee_id, trainer)
    @certification_service.certify(current_user, employee_id, certification_type_id, certification_date, trainer, nil, 0)
  end

  def _recertify(certification, certification_date, trainer)
    @certification_service.recertify(certification, {start_date: certification_date, trainer: trainer})
  end
